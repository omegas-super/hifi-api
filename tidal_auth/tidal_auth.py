...
async def password_login(username: str, password: str, client_id: str = AUTH_CLIENT_ID, client_secret: str = AUTH_CLIENT_SECRET) -> dict:
    """Login with Tidal email/password via OAuth2 password grant and save token to file."""
    headers = {
        "User-Agent": USER_AGENT,
        "Accept": "application/json",
        "Accept-Encoding": "gzip",
...
async def main():
    rich.print("Login method:")
    rich.print("  [1] Device code (browser link) — default")
    rich.print("  [2] Email / Password (account-based, no browser needed)")
    method = input("Choice [1/2]: ").strip()

    if method == "2":
        username = input("Tidal email: ").strip()
        password = input("Tidal password: ").strip()
        try:
            entry = await password_login(username, password)
            rich.print("[green]Login successful![/green]")
            rich.print(entry)
        except Exception as e:
            rich.print(f"[red]Login failed: {e}[/red]")
        return
